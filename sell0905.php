	...
	function sell0905($data, $X, $Z)
	{
		...
		$minuteopen=date('i',$timeopen); //calcul de l'heure d'ouverture en minutes après 9h00
			...
			{			
				...
				if($time>=strtotime("today 9:05am") && $lines[$i][1]<=$max*(1-($X/100))) //si on détecte une perde de plus de X% 
				{
					$time=$timeopen+60*$lines[$i][0];
				...
				if($time>=strtotime("today 9:05am") && $lines[$i][1]<=$price0905*(1-($Z/100))) //si on détecte une perde de plus de Z% après 9h05 
				{
					$time=$timeopen+60*$lines[$i][0];
					$selltime=date("H\hi",$time);
					
					return array("max"=>$price0905." (9h05)","sellprice"=>$lines[$i][1],"selltime"=>$selltime);
				}
